...
export const createEvent = (type: WebSocketEvent['type'], data?: Record<string, unknown>): WebSocketEvent => ({ type, data, timestamp: Date.now() })

export const createDemoGeneratedUI = () => ({
  id: 'wizard-v1', version: '1.0.0', type: 'step_wizard' as const, component: 'StepWizard',
  props: { title: 'A simpler way forward', description: 'A focused flow based on your progress.' },
  fields: ['fullName', 'employmentType', 'annualIncome', 'monthlyExpenses', 'panNumber'], state: {}, timestamp: Date.now(),
})
...
export function useWebSocketPlaceholder() { return { url: WS_URL, mode: 'ready-for-backend' } }
