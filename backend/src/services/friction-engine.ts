import type { FrictionDecision, FrictionLevel, TelemetryEvent } from '../types/telemetry.js'

export class FrictionEngine {
  evaluateTelemetry(event: TelemetryEvent): FrictionDecision {
    const { cursorVelocity, hesitation, repeatedClicks, fieldErrors, activeField } = event.data

    const score = Math.min(
      100,
      Number(cursorVelocity) * 0.35 + Number(hesitation) * 0.25 + Number(repeatedClicks) * 12 + Number(fieldErrors) * 18,
    )

    let level: FrictionLevel = 'LOW'
    if (score >= 75) {
      level = 'HIGH'
    } else if (score >= 45) {
      level = 'MEDIUM'
