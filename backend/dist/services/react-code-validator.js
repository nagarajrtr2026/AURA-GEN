    ...
    while (pending.length > 0) {
        ...
        if (node.type === 'CallExpression') {
            ...
            if (callee?.type === 'Import')
                return { error: 'Dynamic imports are not allowed.', containsJsx };
...
export function validateGeneratedReactCode(source) {
    ...
    if (!result.containsJsx)
        return { approved: false, error: 'Generated React code must contain JSX.' };
